...
async function createProduct(tx: WriteTransaction, input: CreateProduct) {
  ...
  const defaultVariant: Client.ProductVariant = {
...
async function updateProduct(tx: WriteTransaction, input: UpdateProduct) {
  ...
  const product = (await tx.get(id)) as Client.Product | undefined;
...
async function updateProductImagesOrder(
  ...
  const product = (await tx.get(productId)) as Client.Product | undefined;
...
async function createProductOption(
  ...
  const { option } = input;
  const product = (await tx.get(option.productId)) as
    | Client.Product
    | undefined;
...
async function createProductVariant(
  ...
  const { variant } = input;
  const product = (await tx.get(variant.productId)) as
    | Client.Product
    | undefined;
...
async function assignProductOptionValueToVariant(
  ...
  const product = (await tx.get<Client.Product>(productId)) as
    | Client.Product
    | undefined;
  ...
  if (prevOptionValueId)
    optionValues = optionValues.filter(
      (val) => val.value.id !== prevOptionValueId,
    );
